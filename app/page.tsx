...
export default function Home() {
  const picks = [experiences[0], experiences[4], experiences[3]];
  return <main><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify([websiteJsonLd, organizationJsonLd]) }} />
    <section className="home-hero section"><div className="hero-copy"><p className="eyebrow">Curated in Hoi An · Built for real decisions</p><h1>Discover the best of Hoi An — <em>like a local.</em></h1><p>Curated local experiences, activities and places selected for travellers who want more than the usual tourist checklist.</p><div className="hero-actions"><Link className="primary-button" href="#popular">Explore experiences</Link><Link className="secondary-button" href="#plan">Plan my Hoi An day</Link></div><div className="trust-inline"><span>Locally curated</span><span>Independent recommendations</span><span>Trusted booking partners</span></div></div><div className="hero-editorial"><div className="hero-image"><Image src="/tours/my-son.jpg" alt="Historic Cham temples surrounded by greenery at My Son" fill priority sizes="(max-width: 800px) 100vw, 48vw" /></div><div className="editorial-card"><span>Start with your travel style</span><strong>One good half-day can shape the whole trip.</strong><Link href="#find">Find your fit →</Link></div></div></section>
    ...
    <section className="local-picks section"><div className="section-heading"><p className="eyebrow">Local picks</p><h2>Three strong places to start</h2><p>Selected for a clear reason—not because a provider paid for a higher position.</p></div><div className="experience-grid">{picks.map((item, index) => <ExperienceCard key={item.id} experience={item} position={`local-pick-${index + 1}`} label={index === 0 ? "Best overall half-day" : index === 1 ? "Best heritage morning" : "Best first evening"} />)}</div></section>
