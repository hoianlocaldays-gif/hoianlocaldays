...
const kidsCtaIds: Record<string, string> = {
  "/cooking-classes-hoi-an": "kids_cooking",
  "/basket-boat-hoi-an": "kids_basket_boat",
};
...
export function HoiAnWithKidsPage() {
  ...
  return <main className="family-hub">
    ...
    <section className="family-length section"><div className="family-heading"><p className="eyebrow">There is no single correct length</p><h2>How Many Days Do Families Need in Hoi An?</h2><Link className="context-link" href="/how-many-days-in-hoi-an">Compare stay lengths for different traveler types →</Link></div><div className="length-grid">{familyStayLengths.map((item) => <article key={item.title}><h3>{item.title}</h3><p>{item.guidance}</p></article>)}</div></section>
