...
export function ExperienceCard({ experience, page = "homepage", position = "card", label }: { experience: Experience; page?: string; position?: string; label?: string }) {
  return <article className="experience-card" onMouseEnter={() => track("experience_view", { experience_id: experience.id, page })}>
    ...
    <div className="card-body">{label && <span className="pick-label">{label}</span>}<h3>{experience.name}</h3><p className="card-meta">Product details pending verification</p><p className="best-for"><strong>Best for:</strong> {experience.bestFor}</p><p className="reason"><strong>What this page will compare</strong>{experience.editorialReason}</p><div className="card-bottom"><span>No price published</span><Link href="/things-to-do-in-hoi-an#interests" onClick={() => track("experience_view", { experience_id: experience.id, page, position })}>View relevant categories →</Link></div><ProviderButtons experience={experience} page={page} position={position} /></div>
  </article>;
}
